 ...
 * 
 * public static void enableMobBlockBreaking(Mob mob, ServerLevel world, Player
 * target, int ticks) {
 * currentMob = mob;
 * currentWorld = world;
 ...
 * blockX += xDiff > 0 ? 1 : -1;
 * } else if (Math.abs(yDiff) > Math.abs(xDiff) && Math.abs(yDiff) >
 * Math.abs(zDiff)) {
 * blockY += yDiff > 0 ? 1 : -1;
 * } else if (Math.abs(zDiff) > Math.abs(xDiff) && Math.abs(zDiff) >
 * Math.abs(yDiff)) {
 * blockZ += zDiff > 0 ? 1 : -1;
 * }
 * 
 ...
 * if (state.getBlock() != Blocks.AIR && state.getDestroySpeed(currentWorld,
 * blockPos) >= 0) {
 * if (currentTicks < 10) {
 * currentWorld.levelEvent(2001, blockPos, Block.getId(state));
 * } else {
 * currentWorld.destroyBlock(blockPos, true, currentMob);
 * currentWorld.playSound(null, blockPos, SoundEvents.STONE_BREAK,
 * currentMob.getSoundSource(), 1.0F,
 * 1.0F);
