 ...
 * if (server != null) {
 * return server.getWorldData().getLevelName().toLowerCase().replaceAll(" ",
 * "_");
 * } else {
 ...
 * return Paths.get("./config/blockrestoration_" + name + "_" + getWorldName() +
 * ".json");
 * }
 * 
 ...
 * } catch (IOException e) {
 * System.out.println("Error loading initial items data for " + name + ": " +
 * e.getMessage());
 * }
 * return new HashSet<>();
 * }
 ...
 * } catch (IOException e) {
 * System.out.println("Error saving initial items data for " + name + ": " +
 * e.getMessage());
